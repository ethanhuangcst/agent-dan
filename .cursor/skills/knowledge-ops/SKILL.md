...
For **this** project, canonical KB files live in **`./knowledge/`**. Prefer MCP tools `knowledge_search`, `knowledge_read`, `knowledge_write` (**work-agent** server) when implemented; see `agent/architecture.md` and `.cursor/MCP.md`. Classify and dedupe before writing; one canonical path per fact set.
